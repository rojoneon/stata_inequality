*Titulo: 				Cálculo del Método de Medición Integrada de la Pobreza 2016
...
gl bases=	"C:\MMIP2016\Bases Ajustadas"
gl ingresos="C:\MMIP2016\Ingresos"
gl tmp=		"C:\MMIP2016\TMP"
gl final=	"C:\MMIP2016\Final"
...
merge  m:1 folioviv  using "$tmp/viviendas.dta"
drop _merge

*Generar un folio único, de los folios de vivienda y hogares juntos
egen folio=concat(folioviv folioh)
...
keep folio folioviv foliohog municipio entidad ur_rur_2500 alcaldia factor upm tam_loc est_dis
...
*Eliminar los folios de los que no son miembros de la familia en el hogar
...
gen tam_hog=1
collapse (sum) tam_hog, by(folioviv foliohog folio)
save "$tmp/tamano_hogar.dta", replace


*Abrir la base con variables seleccionadas
use "$final/factexp.dta", clear
merge 1:1 folioviv foliohog folio using "$tmp/tamano_hogar.dta"
tab _merge
drop _merge
...
*
* Al final se genera la base "nbi.dta" a partir de hogaresmmip.dta
********************************************************************************
...
***
***2.1.1 Calidad de la vivienda, se elaboran indicadores parciales que miden la calidad de muros,
*** pisos y techos de las viviendas, de acuerdo con los ponderadores que se especifican abajo, 
*** para crear un índice de calidad de la vivienda. En cada componente, se especifica la norma. 
****

****Pared de la vivienda (Norma: Tabique, ladrillo, block, piedra, cantera, cemento o concreto)
...
*Variable para identificar  hogares donde la cocina también es dormitorio, 1 si la cocina es exclusiva, es decir "sí no se usa como dormitorio"
...
*Norma del total de cuartos de la vivienda
...
*Generar adecuación dormitorios por hogar
...
*Generar adecuación espacio de la vivienda
gen AEVh = DEh / DEh_N
...
*Generar la variable de carencia
gen ccevj= 1-acevj
label var ccevj "Ind carencia de espacio y calidad de la vivienda"


******************************************************************
*2.2 Construcción y cálculo del indicador de bienes durables. Este
*indicador compara un valor aproximado de los bienes durables en el 
*hogar, frente a una norma del costo de una canasta de bienes 
*considerados necesarios para el hogar
******************************************************************
...
*D1
*Dato externo 1 = Precios de bienes durables, actualizados a 2016. Véase documento metodológico

*Se le otorga un valor a los bienes de acuerdo a su precio actualizado
*Valores de artículos de vivienda deflactados al mes de agosto de 2016
gen Nauto   =133009.86  	* auto
gen Ncamnet =149066.99 		* camneta
gen Ncamcaj =149066.99 		* camcaj
gen Nmoto   =30196.03 		* moto  
gen Nbici   =1447.53  		* bici
gen Nradio  =616.15 		* radio
gen Ngrabad =616.15 		* grabado
gen Nestereo=1338.33 		* estereo
gen Ntv     =3420.28 		* tv
gen Ndvd 	=492.91  		* dvd
gen Njuegov =5503.43  		* juegovi
gen Ncompu  =11296.98  		* compu
gen Nventi  =329.50 		* venti
gen Nmcoser =2818.11		* mcoser
gen Nestgas =3306.43  		* estgas
gen Nrefri  =5636.06 		* refri
gen Nlicuad =371.99   		* licuad
gen Nbomag  =929.75 		* bomag
gen Nplanch =185.59  		* plancha
gen Nlavado =3895.01 		* lavado
gen Naspira =459.35  		* aspirado
gen Ncalgas =1927.12 		* calgas
...
*Para obtener la carencia se suma de todos los bienes durables y se dividen 
*entre norma, siendo el denominador en 2016, 19208.54
...
*2.3 Construcción y cálculo del indicador de adecuación sanitaria
***********************************************************************
*** Este indicador tiene tres componentes parciales, adecuación de 
*** disponibilidad de agua, conexión al drenaje y excusado

***
***2.3.1 Agua, la norma corresponde a la disponibilidad de agua dentro 
***de la vivienda, con frecuencia diaria.
***
...
*Generar índice de adecuación y frecuencia de agua
gen AA = (aaa + afa)/2
label var AA "Adecuación de abasto hídrico"

***
***2.3.2 Drenaje y excusado, Norma: excusado exclusivo conectado a drenaje
***o fosa séptica
***
...
*La norma es que tengan excusado, no compartido y con descarga directa de agua		<- Norma
gen EX=.
...
gen ASj = AA * ADr * AEX 
label var ASj "Indicador consolidado de adecuación sanitaria"

*Generar indicador de carencia sanitaria
gen CSj = 1 - ASj
label var CSj "Ind de carencia de adecuación sanitaria"
...
* La norma es tener almenos uno de los dos										<- Norma	
...
*La norma es cualquier opción en que sí tengan abasto de energía 
...
* La norma es utilizar gas o electricidad para cocinar 									<- Norma
...
gen AENj= ACK*(0.30) + AElj*(0.70)
label var AENj "Indicador de adecuación energetica"  

*Generación indicador de carencia energética
gen CENJ=1-AENj
label var CENJ "Indicadores de carencia en energetica"
...
keep folio municipio entidad ur_rur_2500 alcaldia factor  tam_hog factorxind ///
...
*Se eliminan los que no son miembros de la familia en el hogardestring edad parentesco, replace 
destring edad parentesco, replace 
drop if parentesco>=400 & parentesco<500
drop if parentesco>=700 & parentesco<800

* Tanto alfabetización (alfabetism) como asistencia escolar (asis_esc)
*	está codificadas de la siguiente manera: 1=si  2 = no
...
* de manera que, en suma, se obtenga el número de años que llevan estudiando, tomando en cuenta preescolar.
...
* se les suma el número de años dependiendo del antecedente escolar
...
*Paso 4: Si el nivel aprobado es profesional, maestría o doctorado, solo se le suman los años que llevan estudiando 
...
gen a_nacimiento= 2016-edad

*Generación variable de norma educativa																	<- Norma
gen 	normaedu=0
...
replace anei_p = 1 + ((anei - 1) / (2.7333-1)) if ( anei>1 & (edad >= 30 & edad  <= 59) ) 
replace anei_p = 1 + ((anei - 1) / (3.1111-1)) if ( anei>1 & edad > 59)
...
keep folioviv foliohog numren clave ing_tri
egen folio= concat(folioviv foliohog)
sort folio numren 
...
*Abrir la base de datos de pensiones a nivel persona
save "$tmp/Pensiones16.dta", replace

*Generar la base de población para unirse a la anterior de pensiones
use "$bases/poblacion.dta", clear

*Se genera la variable folio
egen folio= concat(folioviv foliohog)
sort folio numren 
...
merge 1:1 folio numren using "$tmp/Pensiones16.dta"
drop _merge
...
replace asaludi=0.66 if segpop=="1" | inst_5=="5"
*0.33 si tiene "otro"
replace asaludi=0.33 if inst_6=="6"
...
*****Obtenido de: Cuotas ANUAL IMSS 2016	
************************************************

*Generar variable de "Cuota mensual de IMSS por persona 2016"
gen segimmsi=0
replace segimmsi=(2250)/12 if edad>=0  & edad<=19 
replace segimmsi=(2700)/12 if edad>=20 & edad<=29 
replace segimmsi=(2850)/12 if edad>=30 & edad<=39 
replace segimmsi=(4000)/12 if edad>=40 & edad<=49 
replace segimmsi=(4250)/12 if edad>=50 & edad<=59 
replace segimmsi=(6150)/12 if edad>=60 & edad<=69 
replace segimmsi=(6450)/12 if edad>=70 & edad<=79 
replace segimmsi=(6500)/12 if edad>=80 
label var segimmsi "Cuota IMSS régimen voluntario mensual por persona"
...
* A fin de determinar la cobertura se la SS, sSe crean variable para identificar 
...
* Generar variable (segsocc) que identifica tipo de institución a la que se tiene acceso, para parejas
	* 1 = imss, 2 = issste federal y estatal, 3 = pemex
gen segsocc=0
...
*Paso 2: Se reemplaza de acuerdo a ser asegurado o recibirlo por medio de la pareja, padre/madre o hijas/hijos
...
*Generar los agregados por hogar y se colocan en la misma base a nivel individual
egen cnoderchsalij=   		 sum(cnoderchsali), by (folio)
label var cnoderchsalij "Costo Seguro Voluntario IMSS en salud, suma x hogar"
...
*Se modifica variable de prestación de guardería (prestación_12)
gen pres_12_mod= pres_12
destring pres_12_mod htrab, replace
replace pres_12_mod=0 if  pres_12_mod==.
...
collapse (sum) htrab (max) pres_12_mod, by(folio numren)
rename htrab hstrmesp
label var hstrmesp "Horas trabajadas en ambos empleos"
label var pres_12_mod "Indicador prestación de guardería"
* Guardar base
...
gen guard=0
replace guard=1 if pres_12_mod==12
...
use "${bases}/poblacion.dta", clear 
keep folioviv foliohog edad sexo parentesco
...
gen decena= substr(folioviv,-3,1)
...
*Generar una variable de ingreso identica para hacer modificaciones
gen ing_11=ing_1
gen ing_21=ing_2
gen ing_31=ing_3
gen ing_41=ing_4
...
scalar par_1 =	0.999648
scalar par_2 =	1.001120
scalar par_3 =	0.997950
scalar par_4 =	0.993500
scalar par_5 =	0.994596
scalar par_6 =	0.997189
scalar par_7 =	1.000000
scalar par_8 =	1.006106
scalar par_9 =	1.012212
...
*Para poder hacer el loop se genera el valor numérico de la clave
...
label var g3h_gas_nm_tri "Autoconsumo trim"
label var g5h_gas_nm_tri "Regalos otros hog. trim"
label var g6h_gas_nm_tri "Transferencias de inst. trim"	 
...
***
*6.2.2 Cálculo del ingreso no monetario por persona
***
...
*Control para la frecuencia: se quitan los que no son corrientes, es decir, "otros"=9 y no especificados
drop if ((frec_rem=="9") | frec_rem==" ")
...
recode g4p_gas_nm_tri(.=0)

*Colapsamos la variable a nivel hogar
collapse (sum) g4p_gas_nm_tri, by(folio)

*Se etiqueta la variable
label var g4p_gas_nm_tri "GNM remuneracion en especie trim"

*Guardar la base de datos a nivel persona
save "$ingresos/gastospersona_no_mon.dta", replace 

***
*6.2.3 Cálculo del ingreso no monetario total
***
...
recode g4p_gas_nm_tri g3h_gas_nm_tri g5h_gas_nm_tri g6h_gas_nm_tri g7h_gas_nm_tri (.=0)
...
replace deflac = 1.006106 if meslevan==9
replace deflac = 1.012212 if meslevan==10
replace deflac = 1.020121 if meslevan==11
...
gen reg_hog_tot 	= (((g5h_gas_nm_tri) / deflac)/3)
gen trans_inst_tot  = (((g6h_gas_nm_tri) / deflac)/3)
gen estim_alq   	= (((g7h_gas_nm_tri) / deflac)/3)

*Etiquetar variables
label var autocons		  "GNM Autoconsumo mensual deflactado"
label var remu_esp		  "GNM remu en especie mensual deflactado"
label var reg_hog_tot	  "GNM Reg otros hog. mensual deflactado"
label var trans_inst_tot  "GNM Transf de inst. mensual deflactado"	 
...
*Para el ingreso no monetario mensual, sumar G4 (remu_esp), G5 (regalos recibidos de otros hogares)
	*										   G6 (transf. instituciones) y G7 (estim_alq)  
gen ing_no_mon = (remu_esp + reg_hog_tot + trans_inst_tot + estim_alq) 
label var ing_no_mon "Ing Corr No Mon 8/2016 mensual"
...
recode ing_no_mon ing_mon autocons remu_esp reg_hog_tot trans_inst_tot estim_alq (.=0)

***
*** Se calcula Ingreso Corriente Total
***
gen ict = ing_mon + ing_no_mon 
label var ict "Ingreso Corriente Total Mensual 8/2016"
...
*Se genera ingreso corriente mensual por adulto equivalente
gen ict_ae = ict / ae_sum 
label var ict_ae "Ingreso total mensual / adulto equivalente"

*Se genera ingreso corriente mensual por persona
...
gen LP=0
replace LP = 3587.03 + (760.08 * tam_hog)  + (3166.21 * ae_sum) if ur_rur_2500==1
replace LP = 3065.40 + (709.05 * tam_hog)  + (2844.17 * ae_sum) if ur_rur_2500==2
...
*Generar la variable de adecuación de tiempo verificada por línea de pobreza
gen aytj = ytj / LP
...
*
* Generar base final: final16.dta
* Generar base intermedia: basefinal_pegadas.dta
********************************************************************************
...
gen nbi= (ccevj*.328) + (CSj*.037) + (CENJ*.028) + (CTELJ*.030) +  (CBDj *.058) + (rei*.236) + (CASSi*.1415) + (CASi*.1415)
...
gen E_CSj =.
replace E_CSj=6 if CSj<= -0.5                  
replace E_CSj=5 if (CSj> -0.5 & CSj <= -.1) 
replace E_CSj=4 if (CSj> -0.1 & CSj <= 0) 
replace E_CSj=3 if (CSj> 0 & CSj <= (1/3)) 
replace E_CSj=2 if (CSj> (1/3) & CSj <=.5) 
replace E_CSj=1 if (CSj> .5 & CSj <= 1) 
label var E_CSj "Estrato de adecuación sanitaria"
label values E_CSj estratos
tab E_CSj 
tab E_CSj [fw=factor]
...
keep folioviv foliohog numren tam_loc upm factor folio ur_rur_2500 municipio entidad alcaldia tam_hog factorxind est_dis ///
	...
	ACVj AEVh ccevj ABDj CBDj AA ADr AEX CSj ATlj CTELJ AElj ACK CENJ numren rescgen rei  ///
    ett ing_no_mon  ing_mon inglabth  ict ict_ae LP ae_sum LP_ae ict_ae LP_pc ict_pc  ///
	CASi  CASSi ytj cict ///
	cyt nbi mmip ///
	E_ccevj  E_CBDj E_CSj  E_CTELJ  E_CENJ E_rei ///
	E_CASSi E_CASi  E_nbi E_cict  E_ett E_cyt  E_mmip ///
	pobreza_* pobre_ext*
save "${final}/final16.dta", replace
