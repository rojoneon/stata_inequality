*Titulo: 				Cálculo del Método de Medición Integrada de la Pobreza 2018
...
gl bases=	"C:\MMIP2018\Bases Ajustadas"
gl ingresos="C:\MMIP2018\Ingresos"
gl tmp=		"C:\MMIP2018\TMP"
gl final=	"C:\MMIP2018\Final"
...
*D1
*Dato externo 1 = Precios de bienes durables, actualizados a 2018. Véase documento metodológico

*Se le otorga un valor a los bienes de acuerdo a su precio actualizado
*Valores de artículos de vivienda deflactados al mes de agosto de 2018
gen Nauto   =147909.04  	* auto
gen Ncamnet =165764.82 		* camneta
gen Ncamcaj =165764.72 		* camcaj
gen Nmoto   =33551.21 		* moto  
gen Nbici   =1608.37 		* bici
gen Nradio  =601.00 		* radio
gen Ngrabad =601.00  		* grabado
gen Nestereo=1305.44 		* estereo
gen Ntv     =3220.33		* tv
gen Ndvd 	=504.89  		* dvd
gen Njuegov =5225.74  		* juegovi
gen Ncompu  =10632.24 		* compu
gen Nventi  =355.43 		* venti
gen Nmcoser =3039.85		* mcoser
gen Nestgas =3626.20  		* estgas
gen Nrefri  =6162.36		* refri
gen Nlicuad =398.24   		* licuad
gen Nbomag  =1040.34 		* bomag
gen Nplanch =192.22  		* plancha
gen Nlavado =4167.75 		* lavado
gen Naspira =495.50  		* aspirado
gen Ncalgas =2108.82 		* calgas
...
*entre norma, siendo el denominador en 2018, 20331.90
...
gen     anei_p=anei
replace anei_p = 1 + ((anei - 1) / (1.8   -1)) if ( anei>1 & (edad >= 18 & edad  <= 29) )
...
***Obtenido de: http://www.imss.gob.mx/derechoH/segurosalud-familia		
***Precios originales reportados de Marzo de 2019, llavados a Agosto 2018 con INPC
************************************************

*Generar variable de "Cuota mensual de IMSS por persona 2018"
gen segimmsi=0
replace segimmsi=(3788)/12  if edad>=0  & edad<=19 
replace segimmsi=(4467)/12  if edad>=20 & edad<=29 
replace segimmsi=(4759)/12  if edad>=30 & edad<=39 
replace segimmsi=(6604)/12  if edad>=40 & edad<=49 
replace segimmsi=(6944)/12  if edad>=50 & edad<=59 
replace segimmsi=(10052)/12 if edad>=60 & edad<=69 
replace segimmsi=(10440)/12 if edad>=70 & edad<=79 
replace segimmsi=(10489)/12 if edad>=80 
...
*Se modifica variable de prestación de guardería (prestación_6). Ojo: antes de 2018, era pres_12
...
scalar par_1 =	0.986858
scalar par_2 =	0.990051
scalar par_3 =	0.986694
scalar par_4 =	0.985094
scalar par_5 =	0.988899
scalar par_6 =	0.994200
scalar par_7 =	1.000000
scalar par_8 =	1.004229
scalar par_9 =	1.009434
...
replace deflac = 1.004229192 if meslevan==9
replace deflac = 1.009433587 if meslevan==10
replace deflac = 1.018021335 if meslevan==11
...
label var ing_no_mon "Ing Corr No Mon 8/2018 mensual"
...
label var ict "Ingreso Corriente Total Mensual 8/2018"
...
gen LP=0				
replace LP = 3887.50 + (810.63 * tam_hog)  + (3539.18 * ae_sum) if ur_rur_2500==1
replace LP = 3414.93 + (784.20 * tam_hog)  + (3162.21 * ae_sum) if ur_rur_2500==2
...
keep folioviv foliohog numren tam_loc upm factor folio ur_rur_2500 municipio entidad alcaldia tam_hog factorxind est_dis ///
	...
	pobreza_* pobre_ext*
save "${final}/final18.dta", replace
