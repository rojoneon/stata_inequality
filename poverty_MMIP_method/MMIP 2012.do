*Titulo: 				Cálculo del Método de Medición Integrada de la Pobreza 2012
...
gl bases=	"C:\MMIP2012\Bases Ajustadas"
gl ingresos="C:\MMIP2012\Ingresos"
gl tmp=		"C:\MMIP2012\TMP"
gl final=	"C:\MMIP2012\Final"
...
*Substrae municipio y entidad y se genera una variable para cada una
...
16	"Michoacan"  17	"Morelos" 18	"Nayarit" 19	"Nuevo León" 20	"Oaxaca"  ///
...
*Se seleccionan las variables creados y las de diseño muestral y se genera una nueva base
keep folio folioviv foliohog proyecto municipio entidad ur_rur_2500 alcaldia factor_hog upm tam_loc est_dis
...
*Generar una variable dummy de tamaño de hogar y hacer un collapse para
...
gen factorxind = factor_hog * tam_hog
...
***calidad de muros, pisos y techos de las viviendas, de acuerdo con los ponderadores 
***que se especifican abajo, para crear un índice de calidad de la vivienda. 
***En cada componente, se especifica la norma.
***
...
******************************************************************
*2.2 Construcción y cálculo del indicador de bienes durables. Este indicador 
*compara un valor aproximado de los bienes durables en el hogar, frente a una 
*norma del costo de una canasta de bienes considerados necesarios para el hogar.
******************************************************************

*Se renombran las variables de bienes durables
gen calgas=1 if (calentador== "1")
...
*D1
*Dato externo 1 = Precios de bienes durables, actualizados a 2012. Véase documento metodológico

*Se otorga un valor a los bienes de acuerdo a su precio actualizado
*Valores de artículos de vivienda deflactados al mes de agosto de 2012
gen Nauto   =123494.28     	* auto
gen Ncamnet =138402.68     	* camneta
gen Ncamcaj =138402.68     	* camcaj
gen Nmoto   =27975.81     	* moto
gen Nbici   =1341.10      	* bici
gen Nradio  =607.05     	* radio
gen Ngrabad =607.05     	* grabado
gen Nestereo=1318.59     	* estereo
gen Ntv     =3625.18     	* tv
gen Ndvd    =467.34      	* dvd
gen Njuegov =5884.67     	* juegovi
gen Ncompu  =12671.99     	* compu
gen Nventi  =300.62     	* venti
gen Nmcoser =2571.06     	* mcoser
gen Nestgas =3173.57     	* estgas
gen Nrefri  =5267.65     	* refri
gen Nlicuad =350.64      	* licuad
gen Nbomag  =818.79     	* bomag
gen Nplanch =167.30      	* plancha
gen Nlavado =3666.09     	* lavado
gen Naspira =419.08     	* aspirado
gen Ncalgas =1825.01     	* calgas
...
*norma, siendo el denominador en 2012, 18,560.51
...
*La norma es agua entubada dentro de la vivienda (código 1)
...
*La norma es frecuencia diaria (código 1)
gen frec_agua = 0
...
keep folio proyecto municipio entidad ur_rur_2500 alcaldia factor_hog  tam_hog factorxind ///
...
*SGenerar variable que identifica el año de nacimiento
gen a_nacimiento= 2012-edad
...
*Se generan valores de reescalación
...
***
***Generación de indicador de carencia de educación promedio por hogar
***

*Se genera variable sumada del rezago educativo por hogar, y se pega en la base
egen anei_ph=   		 			sum(anei_p), by (folio)

*Se genera variable de la suma de miembros mayores a dos años por hogar, y se pega en la base
egen mayores_dos_años_h=   sum(mayores_dos_años), by (folio)
label var mayores_dos_años_h "Suma de miembros mayores a dos años por hogar"
...
label var pen044 "Beneficio de programa 70 y más"
...
save "$tmp/Pensiones12.dta", replace
...
merge 1:1 folio numren using "$tmp/Pensiones12.dta"
drop _merge
...
*****Obtenido de: Cuotas ANUAL IMSS 2013, deflactada a pesos de 2012		
************************************************

*Generar variable de Cuota mensual de IMSS por persona
gen segimmsi=0
replace segimmsi=(1371.25)/12 if edad>=0 & edad<=19 
replace segimmsi=(1602.60)/12 if edad>=20 & edad<=39 
replace segimmsi=(2395.40)/12 if edad>=40 & edad<=59 
replace segimmsi=(3604.70)/12 if edad>=60 
label var segimmsi "Cuota IMSS régimen voluntario mensual por persona 2012"
...
*A fin de determinar la cobertura se la SS, se crean variable para identificar a 
*los familiares directos del jefe de familia (pers1) y se aplican las reglas 
...
* Generar la variable "persm", que identifica a la persona que tiene SERVICIO MEDICO DIRECTO por prestacion laboral*.
...
*Generan  base a nivel individual
...
gen pres_6_modificada= pres_6
destring pres_6_modificada htrab, replace
replace pres_6_modificada=0 if  pres_6_modificada==.
...
collapse (sum) htrab (max) pres_6_modificada, by(folio numren)
rename htrab hstrmesp
label var hstrmesp "Horas trabajadas en ambos empleos"
label var pres_6_modificada "Indicador prestación de guardería"
* Guardar base
...
gen guard=0
replace guard=1 if pres_6_modificada==6
...
keep folio proyecto municipio entidad ur_rur_2500 alcaldia cobertura_menores ITDj ///
...
replace  ont = 1 if (motivo_aus == "1" | motivo_aus == "2" | motivo_aus == "3" | motivo_aus == "4" | motivo_aus == "5" | motivo_aus == "6" )
...
gen C_Disc_a=0
replace C_Disc_a=1 if act_discap=="6"
label var C_Disc_a "Personas con discapacidad y que por esa razón no trabajan"
...
************************************************************************
***5.4 Generación de indicadores sobre trabajadores del hogar y 
************************************************************************
...
*
* Generarlas bases intermedias: adultos_equivalentes.dta ing_x_persona.dta
...
use "${bases}/poblacion.dta", clear 
keep folioviv foliohog proyecto edad sexo parentesco
...
scalar par_1 = 0.973655544
scalar par_2 = 0.977147846
scalar par_3 = 0.980960543
scalar par_4 = 0.985101645
scalar par_5 = 0.996315461
scalar par_6 = 0.998630313
scalar par_7 = 1.000000000
scalar par_8 = 1.000937155
scalar par_9 = 0.998830559
...
*Control para la frecuencia: se quitan los que no son corrientes, es decir, 
	...
	*02= Dos veces a la semana
	*03= Tres veces a la semana
	*04= Cuatro veces a la semana
	*05= Cinco veces a la semana
	*06= Seis veces a la semana
	*07= Diario
	*08= Una vez a la quinena
	*09= Una vez al mes
	*10= Cada año
	*11= Lo recibió una sola vez
	*12= Otros
drop if ((frecuencia=="11") | (frecuencia=="12") | (frecuencia==" "))
...
recode g3h_gas_nm_tri g4p_gas_nm_tri g5h_gas_nm_tri g5p_gas_nm_tri /// 
g6h_gas_nm_tri g6p_gas_nm_tri g7h_gas_nm_tri (.=0)
...
replace deflac = 1.000937 if meslevan==9
replace deflac = 0.998830 if meslevan==10
replace deflac = 0.998622 if meslevan==11
...
label var ing_no_mon "Ing Corr No Mon 8/2012 mensual"
...
*Se abre la base plantilla por hogar, y se une a las de ingreso monetario y no monetario creadas
use "$final/factexp.dta",clear 
...
label var ict "Ingreso Corriente Total Mensual 8/2012"
...
gen LP=0
replace LP = 3138.21 + (653.33 * tam_hog) + (2757.05 * ae_sum) if ur_rur_2500==1
replace LP = 2767.27 + (632.63 * tam_hog) + (2484.29 * ae_sum) if ur_rur_2500==2
...
*Gnerar la variable de carencia en Ingresos-tiempo
gen cyt = 1 - aytj_r 
...
*
* Generar base final: final12.dta
* Generar intermedia: basefinal_pegadas.dta
********************************************************************************
...
save "$final/final12.dta", replace
